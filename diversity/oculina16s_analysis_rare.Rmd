...
load("~/oculina/data/taxa2_rev.Rdata")
```
...
#5237 taxa - raw & rarefied

#raw, unrarefied version:
seqtab.clean <- read.csv("oculina16s_rev_seqtab.cleanest.csv",row.names=1)
ps.clean <- phyloseq(otu_table(seqtab.clean, taxa_are_rows=FALSE), 
                    sample_data(samdf), 
                    tax_table(taxa2))
ps.clean
#save(ps.clean,file="ps.clean.rev.Rdata")
...
```{r plotting specificity results}
...
ggplot(df.div.coral, aes(x=Shannon))+
  geom_histogram(binwidth=.2, fill="#69b3a2", color="#e9ecef", alpha=0.9)
shapiro.test(df.div.coral$Shannon)
#log trasform
df.div.coral.log <- df.div.coral
df.div.coral.log$Shannon <- log(df.div.coral.log$Shannon)
ggplot(df.div.coral.log, aes(x=Shannon))+
  geom_histogram(binwidth=.05, fill="#69b3a2", color="#e9ecef", alpha=0.9)
shapiro.test(df.div.coral.log$Shannon)

# I think log transformed data is better here? Should ask a stats person to double check
...
### GLM - Shannon ###
#using Gamma distribution b/c Shannon data is positive and continuous, but not sure if this is best
#or if inverse Gaussian is better or even something else
glm.1 <- glm(Shannon ~ season+site+density, data=df.div.coral.log, family=Gamma())
#summary(glm.1)
glm.2 <- glm(Shannon ~ season+site+density+site*density, data=df.div.coral.log, family=Gamma())
#summary(glm.2)
glm.3 <- glm(Shannon ~ season+site+density+season*site, data=df.div.coral.log, family=Gamma())
#summary(glm.3)
glm.4 <- glm(Shannon ~ season+site+density+season*density, data=df.div.coral.log, family=Gamma())
#summary(glm.4)
glm.5 <- glm(Shannon ~ season+site+density+season*site*density, data=df.div.coral.log, family=Gamma())
#summary(glm.5)
...
#(Intercept)     0.668901   0.085803   7.796  3.7e-09 ***
#seasonSummer    0.069851   0.044588   1.567 0.126208    
#siteNR          0.149260   0.091965   1.623 0.113562    
#siteRI          0.078833   0.087044   0.906 0.371306    
#density         0.004720   0.001510   3.125 0.003564 ** 
#siteNR:density -0.007115   0.001802  -3.949 0.000362 ***
#siteRI:density -0.004040   0.001614  -2.502 0.017157 * 


#df.div.coral.p <- df.div.coral %>% mutate(glm.2 = predict(glm.2))
#ggplot(df.div.coral.p, aes(x=season, y=Shannon))+
#  geom_boxplot()+
#  geom_point(position=position_jitter(height=0,width=0.2))

#ggplot(df.div.coral.p, aes(x=site, y=Shannon))+
#  geom_boxplot()+
#  geom_point(position=position_jitter(height=0,width=0.2))

#ggplot(df.div.coral.p, aes(x=density, y=Shannon,color=site))+
#  geom_point(position=position_jitter(height=0,width=0.2))
...
ggplot(df.div.coral, aes(x=even))+
  geom_histogram(binwidth=.02, fill="#69b3a2", color="#e9ecef", alpha=0.9)
shapiro.test(df.div.coral$even)
#log trasform
df.div.coral.log$even <- log(df.div.coral.log$even)
ggplot(df.div.coral.log, aes(x=even))+
  geom_histogram(binwidth=.02, fill="#69b3a2", color="#e9ecef", alpha=0.9)
shapiro.test(df.div.coral.log$even)
# doesn't look better and shapiro test is worse, so use non-log transformed data

#models:
glm.1 <- glm(even ~ season+site+density, data=df.div.coral, family=Gamma())
#summary(glm.1)
glm.2 <- glm(even ~ season+site+density+site*density, data=df.div.coral, family=Gamma())
#summary(glm.2)
glm.3 <- glm(even ~ season+site+density+season*site, data=df.div.coral, family=Gamma())
#summary(glm.3)
glm.4 <- glm(even ~ season+site+density+season*density, data=df.div.coral, family=Gamma())
#summary(glm.4)
glm.5 <- glm(even ~ season+site+density+season*site*density, data=df.div.coral, family=Gamma())
#summary(glm.5)
...
ggplot(df.div.coral, aes(x=Chao1))+
  geom_histogram(binwidth=20, fill="#69b3a2", color="#e9ecef", alpha=0.9)
shapiro.test(df.div.coral$Chao1) #very non-normal
#log transform
#df.div.coral.log <- df.div.coral
df.div.coral.log$Chao1 <- log(df.div.coral.log$Chao1)
ggplot(df.div.coral.log, aes(x=Chao1))+
  geom_histogram(binwidth=.1, fill="#69b3a2", color="#e9ecef", alpha=0.9) #looks better
shapiro.test(df.div.coral.log$Chao1)
# log-transformed is better

#models:
glm.1 <- glm(Chao1 ~ season+site+density, data=df.div.coral.log, family=Gamma())
#summary(glm.1)
glm.2 <- glm(Chao1 ~ season+site+density+site*density, data=df.div.coral.log, family=Gamma())
#summary(glm.2)
glm.3 <- glm(Chao1 ~ season+site+density+season*site, data=df.div.coral.log, family=Gamma())
#summary(glm.3)
glm.4 <- glm(Chao1 ~ season+site+density+season*density, data=df.div.coral.log, family=Gamma())
#summary(glm.4)
glm.5 <- glm(Chao1 ~ season+site+density+season*site*density, data=df.div.coral.log, family=Gamma())
#summary(glm.5)
...
summary(glm.2)
#                 Estimate Std. Error t value Pr(>|t|)    
#(Intercept)     0.1971460  0.0171038  11.526 1.82e-13 ***
#seasonSummer   -0.0031920  0.0095206  -0.335 0.739417    
#siteNR          0.0372755  0.0181311   2.056 0.047312 *  
#siteRI          0.0208391  0.0172355   1.209 0.234736    
#density         0.0008325  0.0002785   2.990 0.005084 ** 
#siteNR:density -0.0014071  0.0003447  -4.082 0.000246 ***
#siteRI:density -0.0005451  0.0003045  -1.790 0.082127 .
...
```{r MOS test to test for curvilinear relationship}
mod <- MOStest(df.div.coral$density,df.div.coral$Shannon)
isHump(mod)

#idk how to interpret this sooo
```
