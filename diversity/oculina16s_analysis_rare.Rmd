...
# ALPHA DIVERSITY NEEDS TO BE CALCULATED USING RAW DATA - CHANGING ALL PS.RARE TO PS.LESS (9/21/22)
...
df <- data.frame(estimate_richness(ps.less, split=TRUE, measures=c("Shannon","InvSimpson","Observed","Chao1")))
df$id <- rownames(df)
...
library(specificity)
#need to subset just coral samples from ps.less
...
#specs_df <- aggregate_specs_list(specs_list, byFeature=FALSE, fd=seqtab.less, fd_id=1)
...
AICc(glm.0,glm.1a,glm.1b,glm.1c, glm.2a,glm.2b,glm.2c, glm.3a, glm.3b, glm.3c,glm.3d,glm.3e) # AIC 447.42
...
#for lessfied data, remove everything < 2000
row.names.remove <- c("D6","F3","H2","F7","H4","H7")
samdf.less <- samdf.cleanest[!(row.names(samdf.cleanest) %in% row.names.remove), ]

#variable1 = as.character(get_variable(samdf.less, "type"))
#sample_data(ps.all)$type <- mapply(paste0, variable1)
all(rownames(samdf.less) %in% sample_names(ps.all))
samdf.less = sample_data(samdf.less)
merge_phyloseq(ps.all, samdf.less)
